...
2852                     ; 22 void display_touch_debug()
2852                     ; 23 {        //ADC results show
2854                     	switch	.text
2855  0000               _display_touch_debug:
2859                     ; 24         lcd_display_string("READ SUCCESS:",BLACK,GREEN,1,3);
2861  0000 4b03          	push	#3
2862  0002 4b01          	push	#1
2863  0004 ae07e0        	ldw	x,#2016
...
2871                     ; 25         lcd_display_number(14,3,read_res,6);
...
2889                     ; 27         lcd_display_string("X AD Val:",BLACK,GREEN,1,4);
2891  0030 4b04          	push	#4
2892  0032 4b01          	push	#1
2893  0034 ae07e0        	ldw	x,#2016
...
2901                     ; 28         lcd_display_number(10,4,touch_dot.x_ad_val,6);
...
2914                     ; 30         lcd_display_string("Y AD Val:",BLACK,GREEN,1,5);
2916  0056 4b05          	push	#5
2917  0058 4b01          	push	#1
2918  005a ae07e0        	ldw	x,#2016
...
2926                     ; 31         lcd_display_number(10,5,touch_dot.y_ad_val,6);
...
2939                     ; 34         lcd_display_string("X:",BLACK,GREEN,1,6);
2941  007c 4b06          	push	#6
2942  007e 4b01          	push	#1
2943  0080 ae07e0        	ldw	x,#2016
...
2951                     ; 35         lcd_display_number(10,6,touch_dot.x,5);
...
2967                     ; 36         lcd_display_string("Y:",BLACK,GREEN,1,7);
2969  00a7 4b07          	push	#7
2970  00a9 4b01          	push	#1
2971  00ab ae07e0        	ldw	x,#2016
...
2979                     ; 37         lcd_display_number(10,7,touch_dot.y,5);
...
2995                     ; 39 }
2998  00d2 81            	ret
3029                     ; 41 int main(void)
3029                     ; 42 
3029                     ; 43 {
3030                     	switch	.text
3031  00d3               _main:
3035                     ; 47 		CLK_CKDIVR=0x00;
3037  00d3 725f50c6      	clr	_CLK_CKDIVR
3038                     ; 48 		lcd_init();
3040  00d7 cd0000        	call	_lcd_init
3042                     ; 49 		CLK_CKDIVR=0x18;
3044  00da 351850c6      	mov	_CLK_CKDIVR,#24
3045                     ; 50 		TOUCH_init();
3047  00de cd0000        	call	_TOUCH_init
3049                     ; 53 		CLK_CKDIVR=0x00;
3051  00e1 725f50c6      	clr	_CLK_CKDIVR
3052                     ; 56 		lcd_display_string( "experiment:Touch screen    ", BLACK, GREEN, 0, 0 );
3054  00e5 4b00          	push	#0
3055  00e7 4b00          	push	#0
3056  00e9 ae07e0        	ldw	x,#2016
...
3064                     ; 57 		lcd_display_string( "CPU:STM8S20XQ80            ", BLACK, GREEN, 0, 1 );
3066  00f7 4b01          	push	#1
3067  00f9 4b00          	push	#0
3068  00fb ae07e0        	ldw	x,#2016
...
3076                     ; 58 		lcd_display_string( "www.WaveShare.net          ", BLACK, GREEN, 0, 2 );
3078  0109 4b02          	push	#2
3079  010b 4b00          	push	#0
3080  010d ae07e0        	ldw	x,#2016
...
3088  011b               L1502:
3089                     ; 63       read_res=Read_Continue();
3091  011b cd0000        	call	_Read_Continue
3093  011e b700          	ld	_read_res,a
3094                     ; 65 			lcd_draw_bigdot(YELLOW,touch_dot.x,touch_dot.y);
...
3146  001c               L5402:
3147  001c 4350553a5354  	dc.b	"CPU:STM8S20XQ80   "
